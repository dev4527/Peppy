...
function Dashboard() {
  ...
  const moveTaskStatus = async (taskId, currentStatus) => {
    ...
    if (currentIndex === taskStatuses.length - 1) return;
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white dark:bg-[#1e1f21] transition-colors duration-300">
        ...
        <div className="flex-1 overflow-auto bg-[#f9fafb] dark:bg-[#1e1f21] transition-colors duration-300">
          {viewMode === 'project_home' ? (
            ...
            <div className="p-8 grid grid-cols-1 xl:grid-cols-4 gap-5 items-start">
              {taskStatuses.map((status) => {
                const laneTasks = tasks.filter((task) => task.status === status);

                return (
                  <section key={status} className="bg-slate-100 dark:bg-[#252628]/70 border border-slate-200 dark:border-[#333538] rounded-2xl p-4 min-h-56">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-200">{status}</h2>
                      <span className="text-[10px] font-bold text-slate-500 bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-[#333538] rounded-full px-2 py-0.5">
                        {laneTasks.length}
                      </span>
                    </div>
                    ...
                    <div className="space-y-3">
                      {laneTasks.length === 0 ? (
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 italic py-6 text-center">No tasks in this lane.</p>
                      ) : laneTasks.map((task) => (
                        <article
                          key={task._id}
                          onClick={() => setSelectedTask(task)}
                          className="bg-white dark:bg-[#1e1f21] border border-slate-200 dark:border-[#333538] hover:border-red-400 dark:hover:border-red-500 rounded-xl p-4 cursor-pointer shadow-sm transition"
                        >
                          <div className="flex items-start justify-between gap-3">
                            <h3 className="text-sm font-bold text-slate-900 dark:text-white break-words">{task.title}</h3>
                          ...
                          {task.description && (
                            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 line-clamp-2">{task.description}</p>
                          )}
                          ...
                          {status !== 'Completed' && (
                            <button
                              ...
                              onClick={(event) => {
                                ...
                                moveTaskStatus(task._id, task.status);
                              }}
                              className="w-full mt-3 py-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-500 text-[10px] font-black uppercase tracking-wider transition"
                            >
                              Move to {taskStatuses[taskStatuses.indexOf(status) + 1]}
                            </button>
                          )}
                        </article>
                      ))}
            ...
            <div className="p-8 space-y-3">
              {tasks.length === 0 ? (
                <div className="p-10 text-center text-sm text-slate-400 dark:text-slate-500 italic">No tasks found.</div>
              ) : tasks.map((task) => (
                <div
                  key={task._id}
                  onClick={() => setSelectedTask(task)}
                  className="p-4 bg-white dark:bg-[#252628] border border-slate-200 dark:border-[#333538] hover:border-red-400 dark:hover:border-red-500 rounded-xl cursor-pointer transition flex items-center justify-between gap-4"
                >
                  <div className="min-w-0">
                    <p className="font-bold text-sm text-slate-900 dark:text-white truncate">{task.title}</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                      {task.project?.name || currentProject?.name || 'Project task'} · {task.status}
                    </p>
                  </div>
                  {renderRecurrenceChip(task.recurrenceType)}
                </div>
