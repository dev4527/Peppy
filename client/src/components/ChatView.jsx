...
function ChatView() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // ✅ FIXED: Aligned with strict core system authorization mapping standard
      const headers = { Authorization: `Bearer ${token}` };

      // Dynamic path handling: Admins get global logs, Managers/Employees follow dynamic track configurations
      ...
      // 1. Fetch Users & Groups parallel sync pipeline
    ...
    <div className="flex h-[74vh] w-full bg-[#1e1f21] border border-[#2d2e30] rounded-2xl overflow-hidden shadow-2xl animate-fade-in relative z-10">
      ...
      <div className="w-80 bg-[#252628]/60 border-r border-[#2d2e30] flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1.5 custom-scrollbar">
          {activeTab === 'private' ? (
            users.length === 0 ? (
              <p className="text-[10px] text-slate-500 italic p-4 text-center">No other directory employees found.</p>
            ) : (
              users.map((member) => (
                <div 
                  key={member._id}
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-[#848285] font-semibold truncate uppercase">Designation: {member.role}</p>
                  </div>
                </div>
              ))
            )
            ...
            chatGroups.length === 0 ? (
              <p className="text-[10px] text-slate-500 italic p-4 text-center">No active chat channels synchronized.</p>
            ) : (
      ...
      {/* 📱 WHATSAPP NEW GROUP MODAL CREATOR DECK POP-UP */}
      {isGroupModalOpen && (
            ...
            <form onSubmit={handleCreateGroup} className="space-y-3.5">
              ...
              <div>
                ...
                <div className="max-h-28 overflow-y-auto bg-[#151617] border border-[#333538] rounded-xl p-2 space-y-1.5 custom-scrollbar">
                  {users.length === 0 ? (
                    <p className="text-[10px] text-slate-500 italic text-center p-2">No other workspace employees found.</p>
                  ) : (
                    users.map(u => (
